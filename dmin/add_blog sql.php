...
if (isset($_POST['upload'])) {
  ...
  if ($fileInfo['extension'] == "png" or $fileInfo['extension'] == "jpg" or $fileInfo['extension'] == "jpeg") {
    ...
    // Second Snippet with Prepared Statement
    $sql = "INSERT INTO posts (title, description, slug, picture, posted_by, category, date) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $dbcon->prepare($sql);
    $stmt->bind_param("sssssss", $title, $description, $slug, $newFileName, $posted_by, $category, $date);

    if ($stmt->execute()) {
      // echo "<script>window.alert('Blog added successfully!'); window.location='blog.php';</script>";
      echo "<script>window.alert('Blog added successfully!');";
    } else {
